...
get_abund_counts <- function(forcens_df){ 
  
    if(file.exists("data/counts_absolute.csv") && overwrite == FALSE){
    
        raw_counts <- read.csv("data/counts_absolute.csv", header = TRUE)
        return(raw_counts)  
          ...
          for(i in forcens_df$bias_buckley_resample){
              ...
              if (any(!is.na(forcens_df[forcens_df$bias_buckley_resample == i,c("Count_min","Count")]))){
                raw_counts [which(raw_counts[,1]=="total_counts"), sprintf("%sC",i)] <- max(forcens_df[forcens_df$bias_buckley_resample == i,c("Count_min","Count")],na.rm=TRUE)
              }else{ # Count_min and Count are NA
                raw_counts [which(raw_counts[,1]=="total_counts"), sprintf("%sC",i)] <- 100
              }
              
              # Transforming ForCenS rel abund into counts (based on total_counts)
              total_counts <- raw_counts[which(raw_counts[,1]=="total_counts"),sprintf("%sC",i)]
              raw_counts [,sprintf("%sC", i)] <- as.integer(raw_counts [,sprintf("%sC", i)]*total_counts)
              raw_counts [which(raw_counts[,1]=="total_counts"),sprintf("%sC", i)] <- total_counts
              
          }
          
          write.csv(raw_counts, file = "data/counts_absolute.csv",row.names=FALSE)
          return(raw_counts)  
