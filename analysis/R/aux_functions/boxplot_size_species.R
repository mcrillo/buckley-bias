      ...
      if (!file.exists("output/size_boxplot_species") | overwrite == TRUE){
        ...
        if (!file.exists("output/size_boxplot_species")){
            ...
            for(i in unique(data$species)){ # i = c("menardii")
                ...
                b <- ggplot(data.subset, aes(x=sample, y=area_sqrt, fill=dataset)) + geom_boxplot(position = position_dodge(width=dwidth))
                
                pdf(file = paste("output/size_boxplot_species/",i, "_sizebx.pdf", sep=""), paper = "a4r", width = 15, height = 8) # default: inches
                print(b + labs(title = unique(data.subset$sspname), y = "Square-root(area)", x = "Sample ID number") +
                        geom_text(data = data.subset, aes(label = total_ind, y = area_sqrt_max+0.1), size = 6, color="red", position = position_dodge(width=dwidth)) +
